    ...
    IResourceChangeListener
{
  ...
  {
    public MultiPageVelocityEditorrSite(final MultiPageEditorPart mltiPagEdtr,
        final IEditorPart edtr)
    {
  ...
  public VelocityEditorActionContributor getContributor()
  {
    ...
    final VelocityEditorActionContributor ret = (VelocityEditorActionContributor) site
  ...
  protected void pageChange(int newPageIndex)
  {
    ...
    text = vEditor.getDocumentProvider().getDocument(vEditor.getEditorInput())
        .get();
  ...
  private void showPreview(int newPageIndex)
  {
    ...
    // Pattern p = Pattern.compile("(\\#foreach[\\s]*\\(.*[a-z])([\\s]*\\))",
    ...
    // ve.addProperty("velocimacro.library",
    ...
    if (event.getType() == IResourceChangeEvent.PRE_CLOSE)
    {
      Display.getDefault().asyncExec(new Runnable()
      {
        public void run()
        {
          ...
          {
            if (((FileEditorInput) vEditor.getEditorInput()).getFile()
                .getProject().equals(event.getResource()))
            {
              IEditorPart editorPart = pages[i].findEditor(vEditor
                  .getEditorInput());
