...
class Scanner
{
    ...
    {
        if (expandBuffer != null) { return expandBuffer[expandPos]; }
        if (nextChar == 0)
        {
    ...
    public String getUntil(char stopChar) throws IOException
    {
        ...
        while ((ch = read()) >= 0)
        {
            if (ch == stopChar) { return out.toString(); }
            out.append((char) ch);
        }
        return out.toString();
    }
    ...
    public void skipUntil(char stopChar) throws IOException
    {
        ...
        while ((ch = read()) >= 0)
        {
            if (ch == stopChar) { return; }
        }
        return;
    }

    protected Token readNextToken() throws IOException
    {
        for (;;)
        {
            int ch = read();
            if (ch == '<')
            {
                ...
                if (ch == '!')
                {
                    ...
                    {
                        read();
                        if (peekChar() != '-') { throw new DTDParseException(getUriId(), "Invalid character sequence <!-" + read(), getLineNumber(), getColumn()); }
                        read();
                        StringBuffer buff = new StringBuffer();
                        for (;;)
                        {
                            if (peekChar() < 0) { throw new DTDParseException(getUriId(), "Unterminated comment: <!--" + buff.toString(), getLineNumber(), getColumn()); }
                            if (peekChar() != '-')
                            {
            ...
            } else if (ch == '?')
            {
                 ...
                 * ch = peekChar();
                 * 
                 * if (ch == '>') { read(); return new Token(QUESGT); } else {
            ...
            {
                if (read() != ']') { throw new DTDParseException(getUriId(), "Illegal character in input stream: " + ch, getLineNumber(), getColumn()); }
                if (read() != '>') { throw new DTDParseException(getUriId(), "Illegal character in input stream: " + ch, getLineNumber(), getColumn()); }
            ...
            {
                if ((ch == '%') && Character.isWhitespace((char) peekChar())) { return new Token(PERCENT); }
                boolean peRef = (ch == '%');
                StringBuffer buff = new StringBuffer();
                buff.append((char) ch);
                if (isIdentifierChar((char) peekChar()))
                ...
                if (read() != ';') { throw new DTDParseException(getUriId(), "Expected ';' after reference " + buff.toString() + ", found '" + ch + "'", getLineNumber(), getColumn()); }
                buff.append(';');
                if (peRef)
                {
                    ...
                    } else
                    {
                        ...
                        // If the entity can't be expanded, don't return it,
                        // skip it
                        continue;
                    }
                }
            ...
            {
                if (atEOF) { throw new IOException("Read past EOF"); }
                atEOF = true;
            ...
            {
                throw new DTDParseException(getUriId(), "Illegal character in input stream: " + ch, getLineNumber(), getColumn());
            }
        }
    }
    ...
    public void skipConditional() throws IOException
    {
        ...
        for (;;)
        {
            ...
            if (ch == ']')
            {
                ...
                if (ch == ']')
                {
                    ...
                    if (ch == '>')
                    {
                        ...
                        {
                            // We are within an ignoreSectContents section.
                            // Decrement
                            // the nesting depth to represent that this section
                            // has
                            // been ended.
                            nestingDepth--;
                        }
            ...
            // See if this is the first character of the beginning of a new
            // section.
            if (ch == '<')
            {
                ch = read();
    ...
    {
        if (isLetter(ch) || (ch == '_') || (ch == ':')) { return true; }
        return false;
    }
    ...
    {
        if (isLetter(ch) || isDigit(ch) || (ch == '-') || (ch == '_') || (ch == '.') || (ch == ':') || isCombiningChar(ch) || isExtender(ch)) { return true; }
        return false;
    }
    ...
    {
        for (char[] element : letterRanges) {
            if (ch < element[0]) { return false; }
            if ((ch >= element[0]) && (ch <= element[1])) { return true; }
        }
        return false;
    }
    ...
    public boolean isExtender(char ch)
    {
        ...
        if ((ch == 0x00b7) || (ch == 0x02d0) || (ch == 0x02d1) || (ch == 0x0387) || (ch == 0x0640) || (ch == 0x0e46) || ((ch >= 0x3031) && (ch <= 0x3035)) || ((ch >= 0x309d) && (ch <= 0x309e))
    ...
    public void expand(char[] expandChars)
    {
        if (expandBuffer != null)
        {
            ...
            System.arraycopy(expandBuffer, expandPos, newExp, expandChars.length, oldCharsLeft);
            expandPos = 0;
            expandBuffer = newExp;
