...
public class VelocityReconcilingStrategy implements IReconcilingStrategy, IReconcilingStrategyExtension
{
	...
	try {
	    parseHtml();
	 if(fEditor.getEditorInput() instanceof IFileEditorInput)  ((IFileEditorInput)fEditor.getEditorInput()).getFile().deleteMarkers(IMarker.PROBLEM, true, IResource.DEPTH_INFINITE);
	...
	public int getEndLine()
	{
		...
		if (offset == 0)
		    fEditor.addProblemMarker(endTag.getTagName().toLowerCase() + " is not correctly closed proposed line for closing is line " + nrE, nr, IMarker.SEVERITY_WARNING);
	    }
	    return nrE;
	}
